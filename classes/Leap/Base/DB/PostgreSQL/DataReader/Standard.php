 ...
 * @abstract
 */
abstract class Base\DB\PostgreSQL\DataReader\Standard extends DB\SQL\DataReader\Standard {
	...
	public function __construct(DB\Connection\Driver $connection, $sql, $mode = NULL) {
		...
		if ($command === FALSE) {
			throw new Throwable\SQL\Exception('Message: Failed to query SQL statement. Reason: :reason', array(':reason' => @pg_last_error($resource)));
		}
